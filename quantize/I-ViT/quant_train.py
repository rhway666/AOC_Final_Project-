...
def main():
    ...
    if args.resume:
        ...
        logging.info(f"Loaded checkpoint from {args.resume}")
    ...
    # 在 main() 函數中，替換原來的 checkpoint 加載部分（大約第220-235行）
    ...
    #     # 嘗試不同的加載方式
    #     try:
    #         # 方式1: 假設 checkpoint 有 'model' 鍵
    #         model.load_state_dict(checkpoint['model'])
    #         print("Loaded using checkpoint['model']")
    #     except KeyError:
    #         try:
    #             # 方式2: 假設 checkpoint 有 'state_dict' 鍵
    #             model.load_state_dict(checkpoint['state_dict'])
    #             print("Loaded using checkpoint['state_dict']")
    #         except KeyError:
    #             try:
    #                 # 方式3: 假設 checkpoint 直接就是 state_dict
    ...
    #     # 只在非評估模式且包含訓練狀態時加載優化器等
    #     if not args.eval and isinstance(checkpoint, dict):
...
#         model, 
#         {torch.nn.Linear},  # 只對 Linear 層進行量化
#         dtype=torch.qint8   # 使用 8-bit 整數量化
#     )
